...
function MOI.is_valid(opt::Optimizer, c::CI{SAF,T}) where {T <: _C_SETS}
    1 <= c.value <= num_constraints(opt.form.base_form.feasible_region) || return false
...
function MOI.get(opt::Optimizer, ::MOI.ListOfConstraintIndices{SAF,S}) where {S <: _C_SETS}
    indices = CI{SAF,S}[]
...
function MOI.get(opt::Optimizer, ::MOI.ConstraintPrimal, ci::CI{SAF,S}) where {S <: _C_SETS}
    ...
    return MOIU.eval_variables(aff.f) do vi
        MOI.get(opt, MOI.VariablePrimal(), vi)
    end
end
