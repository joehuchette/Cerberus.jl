function down_branch(node::Node, result::NodeResult, branch_vi::MOI.VariableIndex)
    return _branch(node, result, branch_vi, true)
end

function up_branch(node::Node, result::NodeResult, branch_vi::MOI.VariableIndex)
    return _branch(node, result, branch_vi, false)
end

function _branch(node::Node, result::NodeResult, branch_vi::MOI.VariableIndex, branch_down::Bool)
    ...
    return Node(
        ...
        ParentInfo(result.cost, result.basis, nothing),
    )
end

function branch(form::DMIPFormulation, ::MostInfeasible, state::CurrentState, parent_node::Node, result::NodeResult, config::AlgorithmConfig)::Tuple{Node,Node}
    most_frac_idx = 0
    most_frac_val = 1.0
    ...
    up_node = up_branch(parent_node, result, vi)
