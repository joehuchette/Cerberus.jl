@testset "build_base_model" begin
    ...
    @test MOI.get(
        model,
        MOI.NumberOfConstraints{_SAF,_ET}(),
    ) == 1
    c1 = MOI.get(
        model,
        MOI.ListOfConstraintIndices{_SAF,_ET}(),
    )[1]
    ...
    @test MOI.get(
        model,
        MOI.NumberOfConstraints{_SAF,_LT}(),
    ) == 1
    c2 = MOI.get(
        model,
        MOI.ListOfConstraintIndices{_SAF,_LT}(),
    )[1]
    ...
    @test MOI.get(
        model,
        MOI.NumberOfConstraints{_SAF,_GT}(),
    ) == 0

    @test MOI.get(
        model,
        MOI.NumberOfConstraints{_SV,_IN}(),
    ) == 3
    @test MOI.Utilities.get_bounds(model, Float64, _VI(1)) ==
          (0.5, 1.0)
    @test MOI.Utilities.get_bounds(model, Float64, _VI(2)) ==
          (-1.3, 2.3)
    @test MOI.Utilities.get_bounds(model, Float64, _VI(3)) ==
          (0.0, 1.0)
