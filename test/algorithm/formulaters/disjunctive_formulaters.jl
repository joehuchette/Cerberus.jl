@testset "copy_with_new_variable_indices" begin
    ...
    let f = _VOV([_VI(1), _VI(3), _VI(2)])
        ...
        f_actual = @inferred Cerberus._copy_with_new_variable_indices(f, new_vis)
    ...
    let
        ...
        f_expected = MOIU.vectorize([2.0 * _SV(_VI(12)) + 4.0 * _SV(_VI(48)), 6.0 * _SV(_VI(24)) + 8.0 * _SV(_VI(12))])
        f_actual = @inferred Cerberus._copy_with_new_variable_indices(f, new_vis)
        f_actual_scalar = MOIU.scalarize(f_actual)
        @test length(f_actual_scalar) == 2
        @test _is_equal(f_actual_scalar[1], 2.0 * _SV(_VI(12)) + 4.0 * _SV(_VI(48)))
        @test _is_equal(f_actual_scalar[2], 6.0 * _SV(_VI(24)) + 8.0 * _SV(_VI(12)))
    end
end
...
@testset "mask_and_update_variable_indices" begin
    ...
    let
        ...
        masked_disj = @inferred Cerberus.mask_and_update_variable_indices(model_disj, new_vis, mask)
    ...
    let
        ...
        @test _is_equal(f_actual_scalar[1], 2.0 * _SV(_VI(13)) + 4.0 * _SV(_VI(39)))
        @test _is_equal(f_actual_scalar[2], 6.0 * _SV(_VI(26)) + 8.0 * _SV(_VI(13)))
        @test masked_disj.s.lbs == lbs[:, mask]
        @test masked_disj.s.ubs == ubs[:, mask]
    end
end
